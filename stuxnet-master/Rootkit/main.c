...
NTSTATUS HookingFileSystems()
{
     ...
     FunctionAddress = MmGetSystemRoutineAddress(&SystemRoutineName); //get the ObReferenceObjectByName as a function ptr
     if (FunctionAddress == 0)return 0;
     for (i = 0; i < 3;i++){
        HookOne(FunctionAddress,FileSystemsArray[i]); //each call operates on a driver stack
     };
     return STATUS_SUCCESS;
}

VOID HookOne(FUNC,PCWSTR FileSystem) //properly sets fxn_ptr type here
{
     UNICODE_STRING DestinationString;
     NTSTATUS Status;
     PDEVICE_OBJECT AttachObject; //likely an FDO
     ...
     Status = (*ObReferenceObjectByNameFunc)(&DestinationString,0x40,0,0,*IoDriverObjectType,0,0,(PVOID)&FileSystem);
		//ObjectName, Attributes, AccessState=0, DesiredAccess=0, ObjectType, AccessMode=0, ParseContext=0, pObject (out)
		//http://www.codewarrior.cn/ntdoc/wrk/ob/ObReferenceObjectByName.htm
     if (Status!=STATUS_SUCCESS){
        return;
      };
      AttachObject=0;
      AttachObject =((ReferencedObject*)FileSystem)->DeviceObject; //cast to custom struct, pull out object itself
      while (AttachObject != 0)
      {
         DriverNotificationRoutine(AttachObject,1); //this is the only place where we can expect the hooking to actually take place
         AttachObject = (PDEVICE_OBJECT)*((int*)((ULONG)AttachObject + (ULONG)0x0C)); //this reassignment increments, but takes no hooking action
			//Original Comment: Next Element
			//Looks to be adding to a pointer, casting to type int* for DWORD sizing, then dereferencing before casting
      };
      ObDereferenceObject(((ReferencedObject*)FileSystem)); //decrements the reference count to allow for retention checks
};
/**-------------------------------------------------------------------
...
VOID DriverNotificationRoutine(PDEVICE_OBJECT TargetDevice,int command) //actually (at/de)taches device object to/from driver chain/stack
{
    PDEVICE_OBJECT AttachedDevice;
    
    if (command == COMMAND_ATTACH){
        AttachDevice(TargetDevice); //bingo
    }else{
...
BOOLEAN AttachToStack(PDEVICE_OBJECT SourceDevice,PDEVICE_OBJECT TargetDevice,PDEVICE_EXTENSION DeviceExtension)
{
  ...
  if (IoAttachDeviceToDeviceStack(SourceDevice,TargetDevice) == STATUS_SUCCESS){ //since it's a stack, need reference to previous highest device
      return TRUE;  
  };
  return FALSE;
}
